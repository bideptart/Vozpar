...
export async function POST(request: Request) {
  ...
  const html = `
    <h2>New contact enquiry — Vozpar</h2>
  ...
  const ackText = [
    `Hi ${firstName},`,
    "",
    "Thanks for reaching out to Vozpar — we've received your message and a member of our team will get back to you, usually within one business day.",
    "",
    ...
    message,
    "",
    "If it's urgent, just reply to this email or write to support@Vozpar.",
    "",
    "Warm regards,",
    "The Vozpar Team",
  ].join("\n")
  const ackHtml = `
    <div style="font-family:system-ui,-apple-system,sans-serif;font-size:15px;line-height:1.6;color:#171717">
      ...
      <p>Thanks for reaching out to <strong>Vozpar</strong> — we&rsquo;ve received your message and a member of our team
      ...
      <a href="mailto:support@Vozpar" style="color:#DC2626">support@Vozpar</a>.</p>
      <p style="margin-top:20px">Warm regards,<br/>The Vozpar Team</p>
    </div>
  `
  ...
  try {
    ...
    try {
      await transporter.sendMail({
        from: `"Vozpar" <${user}>`,
        to: `"${name}" <${email}>`,
        replyTo: "support@Vozpar",
        subject: "Thanks for contacting Vozpar — we'll be in touch",
        text: ackText,
        html: ackHtml,
      })
    } catch (ackErr) {
    ...
    return NextResponse.json(
      { ok: false, error: "We couldn't send your message right now. Please email support@Vozpar instead." },
      { status: 502 },
    )
  }
}
